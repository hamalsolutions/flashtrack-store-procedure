CREATE DEFINER=`root`@`localhost` PROCEDURE `getSBrecords`(IN numOrder VARCHAR(50))
BEGIN
...
SET @QUERYT = CONCAT("SELECT PRINT.ID as REG,r.* FROM PRINT, JSON_TABLE(DATOS,'$'COLUMNS (",CADENA,")) AS r where PROD_STATUS='1' and ORDER_ID='",numOrder,"';");
